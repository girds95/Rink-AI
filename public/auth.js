...
export async function createProfile({ name, position, age_group, experience_level, makeDefault }) {
  if (!currentUser) {
    // Save locally for anonymous users
    const local = { id: "local", name, position, age_group, experience_level, is_default: true, _local: true };
  ...
  const r = await fetch("/api/profiles", {
    method: "POST", headers,
    body: JSON.stringify({ name, position, age_group, experience_level, makeDefault })
  });
