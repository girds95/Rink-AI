...
const ANON_ID_KEY = "rinkai.anonId";
...
export async function logEvent(event_type, { skill, metadata } = {}) {
  try {
    const headers = { "Content-Type": "application/json", ...(await getAuthHeader()) };
    const body = JSON.stringify({ event_type, anon_id: getAnonId(), skill, metadata });
    // Don't await — analytics shouldn't block UI
    fetch("/api/events", { method: "POST", headers, body }).catch(() => {});
  } catch {}
}
...
export async function createProfile({ name, position, age_group, experience_level, training_access, makeDefault }) {
  if (!currentUser) {
    // Save locally for anonymous users
    const local = { id: "local", name, position, age_group, experience_level, training_access, is_default: true, _local: true };
  ...
  const r = await fetch("/api/profiles", {
    method: "POST", headers,
    body: JSON.stringify({ name, position, age_group, experience_level, training_access, makeDefault })
  });
...
export async function markDrillComplete(sessionId, dayIndex, drillIndex, profileId, track) {
  if (!currentUser || sessionId?.startsWith?.("local-")) {
    ...
    try {
      ...
      // Avoid duplicates — match including track
      if (!arr.some(c => c.session_id === sessionId && c.day_index === dayIndex && c.drill_index === drillIndex && (c.track || null) === trackVal)) {
        arr.push({
  ...
  const r = await fetch("/api/completions", {
    method: "POST", headers,
    body: JSON.stringify({ sessionId, profileId: profileId || currentProfile?.id, dayIndex, drillIndex, track: track || null })
  });
  return r.ok;
}

export async function unmarkDrillComplete(sessionId, dayIndex, drillIndex, track) {
  if (!currentUser || sessionId?.startsWith?.("local-")) {
    try {
      const raw = localStorage.getItem(LOCAL_COMPLETIONS_KEY);
      const arr = raw ? JSON.parse(raw) : [];
      const trackVal = track || null;
      const filtered = arr.filter(c => !(c.session_id === sessionId && c.day_index === dayIndex && c.drill_index === drillIndex && (c.track || null) === trackVal));
