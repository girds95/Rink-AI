...
async function loadHomeData() {
  try {
    ...
    const withWorkout = arr.find(s => s.result?.workout?.days?.length);
    if (withWorkout) {
      state.todaysWorkout = withWorkout;
      const completions = await auth.listCompletions({ profileId });
      const comps = (completions || []).filter(c => c.session_id === withWorkout.id);
      const totalDrills = (withWorkout.result?.workout?.days || [])
        .reduce((sum, d) => sum + (d.drills?.length || 0), 0);
      state.todaysWorkoutPct = totalDrills ? Math.round(comps.length / totalDrills * 100) : 0;
    } else {
...
let profileFormState = { name: "", position: "", age_group: "", experience_level: "", makeDefault: false, editingId: null };
    ...
    pf.addEventListener("submit", async e => {
      ...
      try {
        if (profileFormState.editingId) {
          await auth.updateProfile(profileFormState.editingId, { name: profileFormState.name, position: profileFormState.position, age_group: profileFormState.age_group, experience_level: profileFormState.experience_level, makeDefault: profileFormState.makeDefault });
          toast("Profile updated", "success");
        } else {
          await auth.createProfile({ name: profileFormState.name, position: profileFormState.position, age_group: profileFormState.age_group, experience_level: profileFormState.experience_level, makeDefault: profileFormState.makeDefault });
          toast("Profile created", "success");
        }
        profileFormState = { name: "", position: "", age_group: "", experience_level: "", makeDefault: false, editingId: null };
        go("profile");
      } catch (err) {
...
async function act(a, d) {
  switch (a) {
    ...
    case "edit-profile":
      ...
      if (cur) {
        profileFormState = { name: cur.name||"", position: cur.position||"", age_group: cur.age_group||"", experience_level: cur.experience_level||"", makeDefault: !!cur.is_default, editingId: cur.id };
      }
      go("profileEdit");
      break;
    case "switch-profile":
